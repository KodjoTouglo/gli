...
use vpsguard_core::{CommandRunner, Config, Context, Module, Output, Result, State, UserConfig};
use vpsguard_modules::UsersModule;
...
async fn creates_user_grants_sudo_installs_keys() {
    ...
    let sudoers = tmp.path().join("etc/sudoers.d/vpsguard-deploy");
    assert_eq!(
...
async fn dry_run_writes_nothing() {
    ...
    assert!(!tmp.path().join("etc/sudoers.d/vpsguard-deploy").exists());
