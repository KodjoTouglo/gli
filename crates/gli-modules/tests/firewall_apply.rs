...
use gli_modules::FirewallModule;
...
async fn apply_creates_table_with_guard_rules() {
    ...
    let staged = tmp.path().join("etc/gli/firewall.staged.nft");
...
async fn dry_run_writes_no_script_and_runs_no_apply() {
    ...
    assert!(!tmp.path().join("etc/gli/firewall.staged.nft").exists());
    let calls = runner.calls.lock().unwrap().clone();
...
async fn rollback_replays_snapshot() {
    ...
    tokio::fs::create_dir_all(backup.parent().unwrap())
        .await
        .unwrap();
    tokio::fs::write(&backup, "delete table inet gli\n")
        .await
        .unwrap();
...
async fn disabled_firewall_is_not_applicable() {
    ...
    assert_eq!(status.state, gli_core::State::NotApplicable);
}
