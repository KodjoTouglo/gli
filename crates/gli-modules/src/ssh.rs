...
impl Module for SshModule {
    ...
    async fn uninstall(&self, ctx: &Context, _purge: bool) -> Result<Report> {
        // sshd is not a gli-installed package; "uninstall" reverts the
...
/// The directives gli manages, in the order they should appear.
    ...
    for (key, val) in desired {
        ...
        for line in lines.iter_mut() {
            if is_active_directive(line, key) {
                if placed {
                    *line = format!("# {line}  # superseded by gli");
                } else {
                    *line = target.clone();
                    placed = true;
                }
...
mod tests {
    ...
    fn comments_out_duplicate_actives() {
        ...
        let out = render(input, &desired_directives(&cfg()));
        assert_eq!(active_value(&out, "Port").as_deref(), Some("2222"));
        assert!(out.contains("superseded by gli"));
