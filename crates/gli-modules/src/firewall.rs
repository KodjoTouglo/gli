...
//!
//! gli owns a dedicated `inet gli` table and reprograms it atomically
...
const TABLE: &str = "gli";
const STAGED: &str = "/etc/gli/firewall.staged.nft";
const BACKUP: &str = "/etc/gli/firewall.backup.nft";
...
impl Module for FirewallModule {
    ...
    async fn apply(&self, ctx: &Context, dry_run: bool) -> Result<Report> {
        ...
        report.applied.push(Change::command("nft -f (gli table)"));
        Ok(report)
    }
    ...
    async fn uninstall(&self, ctx: &Context, _purge: bool) -> Result<Report> {
        ...
        // Drop the managed table; leaves the host with no gli firewall.
        let _ = ctx
            .runner()
        ...
        report
            .applied
            .push(Change::command("delete gli nftables table"));
        Ok(report)
    }
}
...
/// `nft list table inet gli`, or `None` when the table does not exist.
